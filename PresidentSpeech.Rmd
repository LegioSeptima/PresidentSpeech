...
get_pct_occur <- function(df, sentiments){
  ...
  n <- nrow(df)/100
  occ <- sentiments.rbind(1:nrow(sentiments)) #2 columns
  foreach(i=1:nrow(sentiments)) %do% occ[1][i] <- (df %>% select("sentiment") %>% filter(sentiment == sentiments[i]))/n
  return(occ)
}
#tmp1 <- top10bussent %>% select("sentiment") %>% filter(sentiment == "joy")

pctocc_bush <- get_pct_occur(top10bussent)
